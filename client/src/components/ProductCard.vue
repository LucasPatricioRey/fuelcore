<script setup>
import { computed } from 'vue'
import { useRouter } from 'vue-router'
import { useCartStore } from '../stores/cart'
import { formatPrice } from '../utils/formatters'
import { getProductImage, getProductTheme, handleProductImageError } from '../utils/productVisuals'

const router = useRouter()
const cartStore = useCartStore()

const props = defineProps({
  product: {
    type: Object,
    required: true,
  },
})

const handleAddToCart = () => {
  cartStore.addItem(props.product)
}

const theme = getProductTheme(props.product.category)

const installmentText = computed(() => {
  const installmentAmount = Math.round(props.product.price / 3)
  return `3 x ${formatPrice(installmentAmount)}`
})
</script>

<template>
  <article class="product-card product-card--commerce" :style="{ '--product-accent': theme.accent, '--product-glow': theme.shadow }">
    <div class="product-card__media product-card__media--commerce">
      <img
        :src="getProductImage(product)"
        :alt="product.name"
        loading="lazy"
        @error="handleProductImageError($event, product)"
      />
      <span v-if="product.featured" class="product-card__badge">Oferta</span>
      <span class="product-card__goal">{{ product.category }}</span>
    </div>

    <div class="product-card__body product-card__body--commerce">
      <div class="product-card__intro">
        <p class="product-card__meta">{{ product.goal }} · stock disponible</p>
        <h3>{{ product.name }}</h3>
        <p class="product-card__description">{{ product.description }}</p>
      </div>

      <div class="product-card__price-block">
        <div>
          <p v-if="product.comparePrice" class="product-card__compare">
            {{ formatPrice(product.comparePrice) }}
          </p>
          <strong>{{ formatPrice(product.price) }}</strong>
        </div>

        <span class="product-card__installments">{{ installmentText }} sin interes</span>
      </div>

      <div class="product-card__footer">
        <span class="product-card__stock">
          <strong>{{ product.stock }}</strong>
          <small>unidades</small>
        </span>
        <span class="product-card__shipping">Envio a todo el pais</span>
      </div>

      <div class="product-card__actions">
        <button class="ghost-button" type="button" @click="router.push(`/productos/${product.slug}`)">
          Ver producto
        </button>
        <button class="primary-button" type="button" @click="handleAddToCart">Agregar al carrito</button>
      </div>
    </div>
  </article>
</template>
